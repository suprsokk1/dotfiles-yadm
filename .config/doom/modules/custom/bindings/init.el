...
(map! (:mode dired-mode "s-."   #'dired-up-directory))
(map! (:mode dired-mode :prefix "." "."  nil))
(map! (:mode dired-mode "." (cmd! (treemacs) ()))) ;FIXME
...
(map! :prefix "s-SPC" "`" #'flycheck-list-errors)

(map! :desc "Toggle writeroom mode" :prefix "s-SPC t" "w"  #'writeroom-mode)
(map! :desc "Toggle writeroom" :prefix "s-SPC t" "w"  #'writeroom-mode)
(map! :desc "Font size +" "s-="  #'doom/increase-font-size)
(map! :desc "Font size -" "s--"  #'doom/decrease-font-size)

(map! :desc "Private config file" "s-SPC c"  #'doom/goto-private-config-file)
