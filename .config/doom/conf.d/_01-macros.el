  ...
  `(get-buffer
    (with-temp-buffer
      ...
      (buffer-substring (match-beginning 0)
                        (match-end 0)))))
;;;###autoload
(defmacro -by-window (WINDOW)
  `(assoc-default ,WINDOW
    (-zip-pair (window-list) (buffer-list))))

;;;###autoload
(defmacro -by-buffer (BUFFER)
  `(assoc-default ,BUFFER
    (-zip-pair (buffer-list) (window-list))))
